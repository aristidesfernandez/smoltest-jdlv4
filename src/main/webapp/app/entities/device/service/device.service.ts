...
type RestOf<T extends IDevice | NewDevice> = Omit<
  T,
  'fabricationDate' | 'endLostCommunication' | 'startLostCommunication' | 'lastUpdateDate' | 'lastCorruptionDate'
> & {
...
export class DeviceService {
  ...
  protected convertDateFromClient<T extends IDevice | NewDevice | PartialUpdateDevice>(device: T): RestOf<T> {
    return {
      ...device,
      fabricationDate: device.fabricationDate?.format(DATE_FORMAT) ?? null,
      endLostCommunication: device.endLostCommunication?.toJSON() ?? null,
      startLostCommunication: device.startLostCommunication?.toJSON() ?? null,
      lastUpdateDate: device.lastUpdateDate?.toJSON() ?? null,
      lastCorruptionDate: device.lastCorruptionDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restDevice: RestDevice): IDevice {
    return {
      ...restDevice,
      fabricationDate: restDevice.fabricationDate ? dayjs(restDevice.fabricationDate) : undefined,
      endLostCommunication: restDevice.endLostCommunication ? dayjs(restDevice.endLostCommunication) : undefined,
      startLostCommunication: restDevice.startLostCommunication ? dayjs(restDevice.startLostCommunication) : undefined,
      lastUpdateDate: restDevice.lastUpdateDate ? dayjs(restDevice.lastUpdateDate) : undefined,
      lastCorruptionDate: restDevice.lastCorruptionDate ? dayjs(restDevice.lastCorruptionDate) : undefined,
    };
  }
